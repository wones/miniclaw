...
Based on miniclaw's design — abstract-property-based Tool ABC with
schema-driven type casting, validation, and OpenAI function-call generation.
"""
...
class Schema(ABC):
    ...
    Concrete types live in :mod:`miniclaw.agent.tools.schema`.
    """
    ...
    def fragment(value: Any) -> dict[str, Any]:
        """Normalize a Schema instance or an existing JSON Schema dict."""
        ...
        if isinstance(value, dict):
            return value
        raise TypeError(
            f"Expected schema object or dict, got {type(value).__name__}"
        )

    @abstractmethod
    def to_json_schema(self) -> dict[str, Any]:
        """Return a JSON Schema fragment dict."""
        ...
...
class Tool(ABC):
    ...
    def validate_params(self, params: dict[str, Any]) -> list[str]:
        ...
        if schema.get("type", "object") != "object":
            raise ValueError(
                f"Schema must be object type, got {schema.get('type')!r}"
            )
